...
export class FamiliesService {
  ...
  private genInviteCode(length = 8): string {
  ...
  async createFamily(name: string, userId: bigint) {
    const now = Date.now();
  ...
  async getMyFamilies(userId: bigint) {
  ...
  async getFamilyDetail(familyId: bigint, familyRole: string) {
  ...
  async updateFamily(familyId: bigint, name: string) {
  ...
  async deleteFamily(familyId: bigint, userId: bigint) {
    const family = await this.prisma.family.findUnique({
      where: { id: familyId },
    });
    ...
    if (family.creatorId !== userId) {
      throw new ForbiddenException('只有创建者才能删除家庭');
    }

    await this.prisma.$transaction(async (tx) => {
      // Delete records associated with the family's plans
  ...
  async generateInviteCode(familyId: bigint) {
    const inviteCode = this.genInviteCode();
  ...
  async joinByInviteCode(inviteCode: string, userId: bigint) {
    ...
    if (existingMember) {
      throw new BadRequestException('您已经是该家庭成员');
    }
  ...
  async getMembers(familyId: bigint) {
  ...
  async updateMemberRole(
    ...
    if (role !== 'admin' && role !== 'member') {
      throw new BadRequestException('角色必须是 admin 或 member');
    }
  ...
  async removeMember(familyId: bigint, targetUserId: bigint) {
    const family = await this.prisma.family.findUnique({
      where: { id: familyId },
    });
  ...
  async leaveFamily(familyId: bigint, userId: bigint) {
    const family = await this.prisma.family.findUnique({
      where: { id: familyId },
    });
    ...
    if (family.creatorId === userId) {
      throw new BadRequestException('创建者不能退出家庭');
    }
  ...
  async addMemberByPhone(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      // Check relationship rule max_count
      ...
      if (rule && rule.maxCount) {
        ...
        if (currentCount >= rule.maxCount) {
          throw new BadRequestException(
            `该关系类型(${relationship})已达到最大数量限制(${rule.maxCount})`,
          );
        }
      }
  ...
  async updateMemberRelationship(
    ...
    return this.prisma.$transaction(async (tx) => {
      // Check relationship rule max_count (excluding current user)
      const rule = await tx.familyRelationshipRule.findUnique({
        where: { relationship },
      });
