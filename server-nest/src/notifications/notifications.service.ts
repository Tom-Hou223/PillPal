...
export class NotificationsService {
  ...
  private wechatTokenExpireTime: number | null = null;
   ...
   * 订阅通知
   */
  async subscribe(userId: number, templateType: string) {
    if (!['expiry', 'medication'].includes(templateType)) {
      throw new BadRequestException('无效的模板类型');
    }

    const templateId = this.configService.get(`wechat.templates.${templateType}`);
    if (!templateId) {
      throw new InternalServerErrorException('模板ID未配置');
    }
   ...
   * 取消订阅
   */
  async unsubscribe(userId: number, templateType: string) {
    const existing = await this.prisma.userSubscription.findUnique({
      where: {
        userId_templateType: {
          userId: BigInt(userId),
  ...
  async getSubscriptions(userId: number) {
    ...
    const expiryTemplate = this.configService.get<string>('wechat.templates.expiry');
    const medicationTemplate = this.configService.get<string>('wechat.templates.medication');
    if (expiryTemplate) templates.expiry = expiryTemplate;
    if (medicationTemplate) templates.medication = medicationTemplate;
   ...
   * 保存用户提醒设置
   */
  async saveSettings(userId: number, dto: SettingsDto) {
    ...
    if (dto.expiryWarningDays !== undefined) data.expiryWarningDays = dto.expiryWarningDays;
  ...
  async getWechatAccessToken(): Promise<string> {
    ...
    throw new InternalServerErrorException(response.data.errmsg || '获取 access_token 失败');
  }
  ...
  async checkAndSendExpiryNotifications() {
    ...
    const expiryTemplateId = this.configService.get<string>('wechat.templates.expiry');
  ...
  async checkAndSendMedicationNotifications() {
    ...
    const medicationTemplateId = this.configService.get<string>('wechat.templates.medication');
