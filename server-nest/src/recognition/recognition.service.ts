...
export class RecognitionService {
  ...
  private baiduTokenExpireTime: number | null = null;
  ...
  private get BarcodeApiUrl(): string {
    return 'https://jumbarcode.market.alicloudapi.com/bar-code/query';
  }

  private get BaiduOcrUrl(): string {
    return 'https://aip.baidubce.com/rest/2.0/ocr/v1/accurate_basic';
  }

  private get BaiduTokenUrl(): string {
    return 'https://aip.baidubce.com/oauth/2.0/token';
  }

  /**
   * Recognize barcode using Aliyun API
   */
  async recognizeBarcode(barcode: string) {
    try {
      const appCode = this.configService.get<string>('aliyun.appCode') || '';
      if (!appCode) {
  ...
  /**
   * Get mock barcode data when API is unavailable
   */
  private getMockBarcodeData(barcode: string) {
    const mockMedicines = [
  ...
  private parseCategoryFromRemark(remark: string): string {
    if (!remark) return '其他';
  ...
  private parseDosageFromRemark(_remark: string): string {
    return '';
  }

  /**
   * Recognize image using Baidu OCR API
   */
  async recognizeImage(imageBuffer: Buffer) {
    try {
      const apiKey = this.configService.get<string>('baidu.apiKey') || '';
      const secretKey = this.configService.get<string>('baidu.secretKey') || '';
  ...
  /**
   * Get Baidu access token with 29-day cache
   */
  async getBaiduAccessToken(): Promise<string> {
    ...
    if (!apiKey || !secretKey) {
      throw new InternalServerErrorException('百度AI API密钥未配置');
    }

    const response = await axios.get(this.BaiduTokenUrl, {
      params: {
    ...
    if (response.data.access_token) {
      this.baiduAccessToken = response.data.access_token as string;
      this.baiduTokenExpireTime = Date.now() + 29 * 24 * 60 * 60 * 1000;
      return this.baiduAccessToken!;
    }
    throw new InternalServerErrorException('获取百度AI access_token 失败');
  }

  /**
   * Parse Baidu OCR result into structured medicine data
   */
  parseOCRResult(ocrData: any) {
    const result: any = {
      name: '',
      manufacturer: '',
      specification: '',
    ...
    const words: string[] = ocrData.words_result.map((item: any) => item.words);
      ...
      if (match && match[1]) {
        ...
        for (const line of lines) {
          ...
          if (line.includes('【') || line.includes('[') ||
            line.includes('用法') || line.includes('用量')) break;
        ...
        if (startIndex !== -1) {
          ...
          if (!needsMore) {
            ...
            for (const end of endings) {
              if (validDosage.endsWith(end)) { needsMore = false; break; }
            }
          }
    ...
    if (hasExpiryToTag) {
      ...
      // Search near label if direct match failed
      if (!expiryDateStr) {
        ...
        for (const label of labelPatterns) {
          ...
          if (idx !== -1) { expiryToIndex = idx; break; }
        }
        ...
        if (expiryToIndex !== -1) {
          ...
          if (nearbyMatch && nearbyMatch[1]) {
            ...
            const allFound: { date: string; index: number }[] = [];
    ...
    // Fallback: scan for date patterns in full text
    if (!expiryDateStr) {
    ...
    if (!expiryDateStr) {
      ...
      for (const pat of allPatterns) {
        ...
        if (m) {
          ...
          for (let i = 1; i < m.length; i++) { if (m[i]) { ds = m[i]; break; } }
          if (!ds) ds = m[0];
          if (ds.length >= 4) {
  ...
  /**
   * Format OCR date string to YYYY.MM.DD
   */
  private formatOcrDate(dateStr: string): string {
    ...
    if (/^\d{8}$/.test(dateStr)) {
      ...
      if (mf) {
        year = mf[1]; month = mf[2].padStart(2, '0'); day = mf[3].padStart(2, '0');
      } else {
        const mm = dateStr.match(/(\d{4})年(\d{1,2})月/);
        if (mm) { year = mm[1]; month = mm[2].padStart(2, '0'); day = '01'; }
      }
      ...
      if (parts.length >= 2) {
        if (parseInt(parts[0]) > 1000) {
          ...
          month = parts[0].padStart(2, '0'); year = parts[1]; day = '01';
        }
      }
    } else if (dateStr.includes('.')) {
      const parts = dateStr.split('.');
    ...
    if (year && (parseInt(year) < 1900 || parseInt(year) > 2100)) return '';
